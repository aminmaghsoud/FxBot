...
#########################  بررسی قدرت کندل خروج    #########################    
            ...
            #### مقداردهی سقف و کف BOS
             if count > 2 : 
                ...
                if round(round(abs(FrameRatesM5.iloc[-2]['high'] - FrameRatesM5['low'].iloc[current_index : -1 ].min()) / (SymbolInfo.point) / 10, 2) / PublicVarible.high_low_diffj  * 1000,1) < 50 : 
                 leg_contorol = 150
            ...
            ## مقداردهی سقف و کف BOS
             if count > 2 : 
              ...
              if  round((round(abs((FrameRatesM5.iloc[current_index : -1]['high'].max()) - ( FrameRatesM5.iloc[-2]['low'])) / (SymbolInfo.point) / 10, 2)) / PublicVarible.high_low_diffj * 1000,1) < 50 :
                 leg_contorol = 150
                 if PublicVarible.high_low_diffj  > (leg_contorol) and PublicVarible.high_low_diffj  < (1200 * ATR_Value) : 
                  PublicVarible.HigherHj = high_C 
                  PublicVarible.LowerLj = low_C  
...
#Buy####################  بررسی شرط خروج قیمت از سقف و انجام معامله خرید ######################
             ...
             if close_C > PublicVarible.Baseroofj and close_C < (PublicVarible.Baseroofj + (SymbolInfo.point * 5)) and PublicVarible.Baseroofj != 0 :
             ...
             elif close_C >= (PublicVarible.Baseroofj + (SymbolInfo.point * 5)) and PublicVarible.Baseroofj != 0 and close_C > HigherHj : 
                ...
                if current_time - PublicVarible.last_execution_timejS  >= 300:   
                   ...
                   Text += f"price:{close_C}$ 🔺Upper Roof {PublicVarible.Baseroofj}$ \n\n "
                   if trend_C == +1 : 
...
#Buy                  
                ...
                if  trend_C == +1 and Time_Signal == 1  : 
                   ...
                   """
                        ...
                        EntryPrice = (PublicVarible.Baseroofj + PublicVarible.Basefloorj) /2
                        if not has_pending :
                           OrderBuyLimit(Pair= self.Pair, Volume= Volume , EntryPrice = EntryPrice , StopLoss= SL, TakeProfit= TP1, Deviation= 0, Comment= "V8 j - M5")
                           PromptToTelegram(f"🚨🚨 \n سفارش #خرید معوق در قیمت \n TP : {TP1} \n Price : {EntryPrice} \n SL : {SL}")
                           PublicVarible.Limittime = current_time
...
#Sell ####################  بررسی شرط خروج قیمت از کف و انجام معامله فروش ######################
             ...
             if close_C < PublicVarible.Basefloorj and close_C > (PublicVarible.Basefloorj + (SymbolInfo.point * 5)) and PublicVarible.Basefloorj != 0 :
                PublicVarible.Baseroofj = PublicVarible.Basefloorj = 0
                ...
                if current_time - PublicVarible.last_execution_timejS >= 300:   
                   ...
                   Text += f"price:{close_C}$ 🔻Under floor {PublicVarible.Basefloorj}$ \n\n "
                   if trend_C == -1 : 
