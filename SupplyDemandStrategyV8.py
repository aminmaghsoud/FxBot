...
##############################################################################################################################################################
      def Main(self):
          ...
          print (Fore.LIGHTCYAN_EX,Back.BLACK ,"--------------", self.Pair,Back.RESET,Fore.RESET,"------------------ Strategy V8 M5 DowJones30 ")
          # ارسال پیام
...
#########################  بررسی قدرت کندل خروج    #########################    
             ...
             high_low_diff = 0.0
             Text = None
             if (high_C > (high_C_O + (SymbolInfo.point * 5))) :# and (FrameRatesM5.iloc[-2]['low'] > FrameRatesM5.iloc[-3]['low']) : 
            ...
            #### مقداردهی سقف و کف BOS
             if count > 2 : 
                ...
                if round(round(abs(FrameRatesM5.iloc[-2]['high'] - FrameRatesM5['low'].iloc[current_index : -2 ].min()) / (SymbolInfo.point) / 10, 2) / high_low_diff * 1000,1) < 50 : 
                 if high_low_diff > (50 * ATR_Value * 0.9) and high_low_diff < (1200 * ATR_Value) : 
                  PublicVarible.List_of_high = high_C 
                  PublicVarible.List_of_low = low_C 
                  if low_C < low_C_O : 
                     PublicVarible.Basefloor5 = low_C
                  elif low_C_O < low_C : 
                     PublicVarible.Basefloor5 = low_C_O
                  else : PublicVarible.Basefloor5 = low_C_O
             ...
             high_low_diff = 0.0
             Text = None       
             if (low_C < low_C_O - (SymbolInfo.point * 5)) :# and (FrameRatesM5.iloc[-2]['high'] < FrameRatesM5.iloc[-3]['high']) :
                   while current_index > end_index : 
                       Now_c_H = FrameRatesM5.iloc[current_index]['high']
                       Old_c_H = FrameRatesM5.iloc[current_index - 1]['high'] 
                       Now_c_L = FrameRatesM5.iloc[current_index]['low']
             ...
             if count > 2 : 
                ...
                if round((round(abs((FrameRatesM5.iloc[current_index : -2]['high'].max()) - ( FrameRatesM5.iloc[-2]['low'])) / (SymbolInfo.point) / 10, 2)) / high_low_diff * 1000,1) < 50 :
                 if high_low_diff > (50 * ATR_Value * 0.9) and high_low_diff < (1200 * ATR_Value) :
                  ...
                  PublicVarible.List_of_low = low_C  
                  if high_C > high_C_O : 
                     PublicVarible.Baseroof5 = high_C
                  elif high_C_O > high_C : 
                     PublicVarible.Baseroof5 = high_C_O
                  else : PublicVarible.Baseroof5 = high_C_O
...
#Buy
                ...
                SL = PublicVarible.Basefloor5 - ((PublicVarible.Baseroof5 - PublicVarible.Basefloor5)/2) #* ( SymbolInfo.point * 30)   #########  تعیین حدضرر معامله #########
...
#Sell
                ...
                SL = PublicVarible.Baseroof5 + ((PublicVarible.Baseroof5 - PublicVarible.Basefloor5)/2) #( SymbolInfo.point * 50)        #########  تعیین حدضرر معامله #########
                ...
                if (abs(close_C - PublicVarible.Basefloor5) < (abs(PublicVarible.Baseroof5 - PublicVarible.Basefloor5)* 0.75) ) and (trend_C == -1 ) and Time_Signal == 1 : #and PublicVarible.hmaSignal == -1:
