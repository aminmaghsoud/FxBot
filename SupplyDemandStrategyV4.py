...
##############################################################################################################################################################
      def Main(self):
          ...
          if SymbolInfo is not None :
             ...
             trendN ,  final_confidence = analyze_market_power(FrameRatesM5, FrameRatesM15, FrameRatesM30) 
             print("trendN: ",trendN , "final_confidence: " ,final_confidence )

             PairNameN = "دلار کانادا/ ین ژاپن"
...
########################################################################################### دریافت اطلاعات تایم فریم ها و محاسبه اندیکاتور #########################################################################################################
             Balace = GetBalance()
             if current_time - PublicVarible.BasetimeN >= 2100 and PublicVarible.BasetimeN != 0 and PublicVarible.BasefloorN != 0: 
                PublicVarible.BaseroofN = PublicVarible.BasefloorN = 0  
                #PromptToTelegram(f"⚠️ بعلت طولانی شدن خروج قیمت از ناحیه رنج ، ناحیه حذف شد!")
                PublicVarible.BasetimeN = 0
             ...
             #print("PublicVarible.BasetimeN:",PublicVarible.BasetimeN)
...
#########################  بررسی قدرت کندل خروج    #########################    
             LowerLN = PublicVarible.LowerLN
             HigherHN = PublicVarible.HigherHN
             #print(f"Lower low = {PublicVarible.LowerLN} \nhigher high = {PublicVarible.HigherHN}")
             ...
             print(f"\n BaseroofN : {PublicVarible.BaseroofN}")
             print("Close -2 : " , close_C)
             print("BasefloorN : " , PublicVarible.BasefloorN)
            ...
            #### مقداردهی سقف و کف BOS
             if count > 2 : 
                PublicVarible.Leg_startN = FrameRatesM5.iloc[current_index]['high']
                PublicVarible.high_low_diffN  = round((abs( FrameRatesM5['low'].iloc[current_index : -1 ].min() - FrameRatesM5.iloc[current_index]['high'])) / (SymbolInfo.point),2)
                if round(round(abs(FrameRatesM5.iloc[-2]['high'] - FrameRatesM5['low'].iloc[current_index : -1 ].min()) / (SymbolInfo.point) / 10, 2) / PublicVarible.high_low_diffN  * 1000,1) < 60 : 
                 leg_contorol = 150
                 if PublicVarible.high_low_diffN  > (leg_contorol) and PublicVarible.high_low_diffN  < (1200 * ATR_Value) : 
                  PublicVarible.HigherHN = high_C 
                  PublicVarible.LowerLN = low_C 
                  PublicVarible.BasefloorN = FrameRatesM5['low'].iloc[current_index : -1 ].min() 
                  PublicVarible.BaseroofN = FrameRatesM5.iloc[-2]['high']
                  PublicVarible.BasetimeN = current_time
                  PublicVarible.range_heightN = round(abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN) / (SymbolInfo.point) / 10, 2)
                  print(f"Down PublicVarible.high_low_diffN : {PublicVarible.high_low_diffN } and BaseroofN: {PublicVarible.BaseroofN} and BasefloorN: {PublicVarible.BasefloorN} and Range arraye: {abs(PublicVarible.BasefloorN - PublicVarible.BaseroofN) / (SymbolInfo.point)} \n")
                  current_time = time.time()
                  if current_time - PublicVarible.last_execution_timeN >= 300:  
                   Text = f"{PairNameN}\n"
                   Text += f"{self.Pair} Price is ({SymbolInfo.ask} $)\n"
                   Text += f"M5️⃣ لگ نزولی و رنج# ... 🔴🔴 \n"
                   Text += f"تعداد کندل: {count}\n"
                   Text += f"ارتفاع لگ: {round(PublicVarible.high_low_diffN , 2) / 10} pip\n"
                   Text += f"ارتفاع رنج: {PublicVarible.range_heightN} pip \n"
                   Text += f"نسبت رنج به لگ: {round(PublicVarible.range_heightN / PublicVarible.high_low_diffN  * 1000,1) } % \n"
                   Text += f"سقف رنج: {PublicVarible.BaseroofN} $ \n"
                   Text += f"کف رنج : {PublicVarible.BasefloorN} $ \n"
                   Text += f"حجم کل مجاز : {round((Balace * 0.8) * (PublicVarible.risk/1000) / PublicVarible.range_heightN , 2)} Lot \n"
                   ...
                   PublicVarible.last_execution_timeN = current_time
            ...
            ## مقداردهی سقف و کف BOS
             if count > 2 : 
              PublicVarible.Leg_startN = FrameRatesM5.iloc[current_index]['low']
              PublicVarible.high_low_diffN  = round((abs(FrameRatesM5.iloc[current_index : -1]['high'].max() - FrameRatesM5.iloc[current_index]['low'])) / (SymbolInfo.point) , 2)
              if  round((round(abs((FrameRatesM5.iloc[current_index : -1]['high'].max()) - ( FrameRatesM5.iloc[-2]['low'])) / (SymbolInfo.point) / 10, 2)) / PublicVarible.high_low_diffN * 1000,1) < 60 :
                 leg_contorol = 150
                 if PublicVarible.high_low_diffN  > (leg_contorol) and PublicVarible.high_low_diffN  < (1200 * ATR_Value) : 
                  PublicVarible.HigherHN = high_C 
                  PublicVarible.LowerLN = low_C  
                  PublicVarible.BaseroofN = FrameRatesM5.iloc[current_index : -1]['high'].max()
                  PublicVarible.BasefloorN = FrameRatesM5.iloc[-2]['low']
                  PublicVarible.BasetimeN = current_time
                  PublicVarible.range_heightN = round(abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN) / (SymbolInfo.point) / 10, 2)
                  print(f"Up PublicVarible.high_low_diffN : {PublicVarible.high_low_diffN } and BaseroofN: {PublicVarible.BaseroofN} and BasefloorN: {PublicVarible.BasefloorN} and Range arraye: {abs(PublicVarible.BasefloorN - PublicVarible.BaseroofN) / (SymbolInfo.point)} \n")
                  current_time = time.time()
...
########################  پیداکردن بالاترین سقف و پایین ترین کف رنج   ################################
             ...
             if PublicVarible.BaseroofN != 0 and close_C < PublicVarible.BaseroofN and close_C > PublicVarible.BasefloorN : 
               if high_C > PublicVarible.HigherHN : 
                  PublicVarible.HigherHN = high_C 
               if low_C < PublicVarible.LowerLN: 
                  PublicVarible.LowerLN = low_C
             elif PublicVarible.BasefloorN == 0 : 
                  PublicVarible.LowerLN = PublicVarible.HigherHN  = 0
...
################################### بررسی الگوی سر و شانه #####################################
             ...
             if PublicVarible.BasefloorN == 0 : PublicVarible.HS_UpN = PublicVarible.HS_DownN = 0 
             elif PublicVarible.BasefloorN != 0 and PublicVarible.HS_UpN == 0 and PublicVarible.HS_DownN == 0 : 
               if (CH4 < CH3 and CH3 > CH2 and CC2 < CL3 and CC2 < CL4) or ((CC3 >= CL4 or CC3 >= CL5 ) and (CH5 < CH4 and CH4 > CH3 and CC2 < CL4 and CC2 < CL5 and CC2 < CL3)): 
                     ...
                     PublicVarible.HS_UpN = 1 

#Buy####################  بررسی شرط خروج قیمت از سقف و انجام معامله خرید ######################
             
             if close_C > PublicVarible.BaseroofN and close_C < (PublicVarible.BaseroofN + (SymbolInfo.point * 5)) and PublicVarible.BaseroofN != 0 :
                PublicVarible.BaseroofN = PublicVarible.BasefloorN = 0
             ...
             elif close_C >= (PublicVarible.BaseroofN + (SymbolInfo.point * 1)) and PublicVarible.BaseroofN != 0 and close_C > HigherHN : 
                print(f"price is {close_C} and Upper Roof {PublicVarible.BaseroofN} ")
                if current_time - PublicVarible.last_execution_timeNS  >= 300:   
                   Text = f"\n({PairNameN}) \n⬆️ Buy Position in {self.Pair} \n"
                   Text += f"price:{close_C}$ \n🔺Upper Roof {PublicVarible.BaseroofN}$ \n\n"
                   if trend_C == +1 : 
                       Text += f"🔘خروج  از سقف:  کندل قدرتمند 🐮 \n"
                       if PublicVarible.HS_DownN == 1 : 
                          Text += f"🔘 الگوی سر وشانه نزولی \n"
                       elif PublicVarible.HS_UpN == 1 : 
                   ...
                   if final_confidence < 65 :
                      ...
                      Text += f"\n✅ ضریب اطمینان پایش مناسب است ({round(final_confidence , 2)}) "
                   if trend_C == 1 and trendN == 1 : 
                      Text += f"\n✅ موقعیت Buy: مناسب "
                   else : 
                      Text += f"\n❌ موقعیت Buy: نامناسب "
                   plot_candles_and_send_telegram(FrameRatesM5, self.Pair, Text)
                   PublicVarible.last_execution_timeNS = current_time 
#Buy            
                EntryPrice = SymbolInfo.ask
                SL = PublicVarible.BasefloorN - ( SymbolInfo.point * 100)  #########  تعیین حدضرر معامله #########
                #TP1 = EntryPrice + ((EntryPrice - SL) * 1  )
                TP1 =  PublicVarible.BaseroofN + (abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN) *   2) 
                Entryheight = round(abs(EntryPrice - PublicVarible.BasefloorN) / (SymbolInfo.point) / 10, 2)      
                ...
                if  trend_C == +1 and trendj == +1 and Time_Signal == 1  : 
                   if  (abs(close_C - PublicVarible.BaseroofN) < (abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN) * 0.75 )):       
                     ...
                     #OrderBuyLimit(Pair= self.Pair, Volume= round(Volume/2 ,2) , EntryPrice =  PublicVarible.BasefloorN , StopLoss= SL, TakeProfit= TP1, Deviation= 0, Comment= "Lim V5")
...
#Sell ####################  بررسی شرط خروج قیمت از کف و انجام معامله فروش ######################
             ...
             if close_C < PublicVarible.BasefloorN and close_C > (PublicVarible.BasefloorN + (SymbolInfo.point * 1)) and PublicVarible.BasefloorN != 0 :
                PublicVarible.BaseroofN = PublicVarible.BasefloorN = 0
                Text = f" مقدار و قدرت خروج قیمت از کف #نامناسب است \n 🔘 حذف مقادیر سقف و کف ⚠️"
                #results = send_telegram_messages(Text, PublicVarible.chat_ids)

             elif close_C <= (PublicVarible.BasefloorN - (SymbolInfo.point * 5)) and PublicVarible.BasefloorN != 0 and close_C < LowerLN : 
                print(f"price is {close_C} and Under floor {PublicVarible.BasefloorN} ")
                if current_time - PublicVarible.last_execution_timeNS >= 300:   
                   Text = f"\n({PairNameN}) \n⬇️ Sell Position in {self.Pair} \n"
                   Text += f"🔘price:{close_C}$ \n🔻Under floor {PublicVarible.BasefloorN}$ \n\n"
                   if trend_C == -1 : 
...
#Sell
                ...
                SL = PublicVarible.BaseroofN + ( SymbolInfo.point * 100)  #((PublicVarible.BaseroofN - PublicVarible.BasefloorN)/2)                     #########  تعیین حدضرر معامله #########
                   #TP1 = EntryPrice + ((EntryPrice - SL) * 1  )
                TP1 = PublicVarible.BasefloorN- (abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN) * 2) 
                Entryheight = round(abs(EntryPrice - PublicVarible.BaseroofN) / (SymbolInfo.point) / 10, 2)      
                Volume = round((Balace * 0.8) * (PublicVarible.risk/1000) / Entryheight , 2)
                TextN = f"\nVolume = {Volume} \n"
                ...
                if  trend_C == -1  and trendj == -1 and Time_Signal == 1 :
                   if  (abs(close_C - PublicVarible.BasefloorN) < (abs(PublicVarible.BaseroofN - PublicVarible.BasefloorN)* 0.75) ) :
                     ...
                     #OrderSellLimit(Pair= self.Pair, Volume=  round(Volume/2 ,2) , EntryPrice =  PublicVarible.BaseroofN , StopLoss= SL, TakeProfit= TP1, Deviation= 0, Comment= "Lim V5")
